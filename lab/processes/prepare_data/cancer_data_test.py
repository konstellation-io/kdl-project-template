...
from lab.processes.prepare_data.cancer_data import (
    load_cancer_data,
    load_data_splits,
...
class TestCancerDataPreparation:
    ...
    def test_split_data_returns_expected_objects(self):
        ...
        assert len(result) == 6, "Expected split_data to return 6 objects (train/test/split for each of X and y)"
    ...
    def test_prepare_cancer_data_saves_parquet_files(self):
        """
        Test that prepare_cancer_data saves arrays as .gzip files
        """

        dir_output = "temp/data"  # Temporarily save the resulting files generated by running the function
        dir_input = "lab/data/raw/template_data"

        # Before calling prepare_cancer_data, check the folder does not exist (necessary for validity of test)
        assert dir_output not in os.listdir()
        time.sleep(1)

        prepare_cancer_data(dir_input=dir_input, dir_output=dir_output)
        ...
        assert dir_output == "temp/data"  # Prevent catastrophe if changing directory by mistake
...
class TestCancerDataLoading:
    ...
    def test_load_data_splits_as_npy_array(self, temp_data_dir):
        ...
        Note: requires dir_temp populated with .gzip files as generated by prepare_cancer_data, prepared by
